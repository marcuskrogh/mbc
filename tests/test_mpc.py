...
class TestDiscreteLinearOCP:
    ...
    def _make_ocp(self, N=10, **kw):
        ...
        return DiscreteLinearOCP(model, N=N, Q=np.eye(1), R=np.eye(1) * 0.1, y_offset=20.0, **kw)

    def test_solve_returns_correct_shapes(self):
        ocp = self._make_ocp(N=5)
        model = DoubleIntegrator()
        x0 = np.array([0.0, 0.0])
        D = np.zeros(5 * model.nd)
        x_ref = model.x_ref
    ...
    def test_solve_drives_toward_reference(self):
        ...
        x0 = np.array([0.0, 0.0])
        x_ref = model.x_ref
        D = np.zeros(20 * model.nd)
        _, X = ocp.solve(x0, D, x_ref)
        X_np = X.reshape(20, model.nx)
    ...
    def test_input_bounds_respected(self):
        ...
        model = DoubleIntegrator()
        x0 = np.array([0.0, 0.0])
        D = np.zeros(10 * model.nd)
        x_ref = model.x_ref
        U, _ = ocp.solve(x0, D, x_ref)
        u_min, u_max = model.u_bounds
        assert np.all(U >= u_min[0] - 1e-6)
        assert np.all(U <= u_max[0] + 1e-6)
    ...
    def test_rate_penalty_reduces_variation(self):
        ...
        ocp_no_rom = DiscreteLinearOCP(model, N=N, Q=np.eye(1), R=np.eye(1) * 0.01, y_offset=20.0)
        ocp_rom = DiscreteLinearOCP(
            model, N=N, Q=np.eye(1), R=np.eye(1) * 0.01, y_offset=20.0,
            S=np.eye(1) * 10.0,
        )
        U_no_rom, _ = ocp_no_rom.solve(x0, D, x_ref)
        U_rom, _ = ocp_rom.solve(x0, D, x_ref)

        u_no_rom = U_no_rom.reshape(N, 1)
        u_rom = U_rom.reshape(N, 1)
...
class TestMPCController:
    ...
    def _make_ctrl(self, N=10):
        ...
        ocp = DiscreteLinearOCP(model, N=N, Q=np.eye(1), R=np.eye(1) * 0.1, y_offset=20.0)
    ...
    def test_closed_loop_drives_toward_reference(self):
        ...
        kf = KalmanFilter(model)
        ocp = DiscreteLinearOCP(model, N=10, Q=np.eye(1) * 5.0, R=np.eye(1) * 0.1, y_offset=20.0)
        ctrl = MPCController(model, estimator=kf, ocp=ocp)
...
class TestContinuousLinearOCP:
    ...
    def _make_ocp(self, N=10):
        ...
        return ContinuousLinearOCP(model, N=N, Q=np.eye(1), R=np.eye(1) * 0.1, y_offset=10.0), model

    def test_solve_returns_correct_shapes(self):
        ocp, model = self._make_ocp(N=5)
        x0 = np.array([0.0])
        D = np.zeros(5 * model.nd)
        U, X = ocp.solve(x0, D, model.x_ref)
        assert U.shape == (5 * model.nu,)
        assert X.shape == (5 * model.nx,)
    ...
    def test_solve_input_within_bounds(self):
        ...
        x0 = np.array([0.0])
        D = np.zeros(8 * model.nd)
        U, _ = ocp.solve(x0, D, model.x_ref)
        u_min, u_max = model.u_bounds
        assert np.all(U >= u_min[0] - 1e-6)
        assert np.all(U <= u_max[0] + 1e-6)
    ...
    def test_solve_drives_toward_reference(self):
        ...
        x0 = np.array([0.0])
        D = np.zeros(20 * model.nd)
        _, X = ocp.solve(x0, D, model.x_ref)
        ref_val = model.x_ref[0]
        dist_start = abs(x0[0] - ref_val)
        dist_end = abs(X[-1] - ref_val)
        assert dist_end < dist_start, (
            f"Final state {X[-1]:.3f} not closer to ref {ref_val:.1f}"
        )

    def test_numpy_x0_accepted(self):
        """solve() accepts a numpy x0."""
        ocp, model = self._make_ocp(N=5)
        x0 = np.array([0.5])
        D = np.zeros(5 * model.nd)
        U, X = ocp.solve(x0, D, model.x_ref)
...
class TestCDMPCController:
    ...
    def _make_ctrl(self, N=10):
        ...
        ocp = ContinuousLinearOCP(model, N=N, Q=np.eye(1), R=np.eye(1) * 0.1, y_offset=10.0)
    ...
    def test_closed_loop_drives_toward_reference(self):
        ...
        kf = ContinuousDiscreteKalmanFilter(model, n_steps=10)
        ocp = ContinuousLinearOCP(model, N=20, Q=np.eye(1) * 5.0, R=np.eye(1) * 0.01, y_offset=10.0)
        ctrl = CDMPCController(model, estimator=kf, ocp=ocp)
...
# ── Tests: ContinuousNonlinearOCP (tracking) ─────────────────────────────────


class TestContinuousNonlinearOCPTracking:
    """Tests for ContinuousNonlinearOCP in tracking mode (SLSQP NLP solver)."""

    def _make_ocp(self, N=5, **kw):
        model = ScalarNonlinear()
        Q = np.eye(1)
        R = np.eye(1) * 0.1
        return ContinuousNonlinearOCP(
...
# ── Tests: ContinuousNonlinearOCP (economic) ─────────────────────────────────


class TestContinuousNonlinearOCP:
    """Tests for the ContinuousNonlinearOCP (economic/general nonlinear OCP)."""
    ...
    def test_mayer_term_is_included(self):
        ...
        ocp_no_mayer = ContinuousNonlinearOCP(
        ...
        ocp_mayer = ContinuousNonlinearOCP(
...
class TestContinuousNonlinearOCPOnSDAE:
    """ContinuousNonlinearOCP's direct-simultaneous formulation must satisfy g(x, y, p) = 0
...
class TestAnalyticalJacobians:
    ...
    def test_cdtracking_ocp_has_analytical_jac(self):
        """ContinuousNonlinearOCP with R_stage and P_terminal always has analytical Jacs."""
        model = ScalarNonlinear()
        ocp = ContinuousNonlinearOCP(
            model, N=3,
            Q_z=np.eye(1) * 2.0,
        ...
        assert ocp._can_use_analytical_objective_jac(), (
            "ContinuousNonlinearOCP with R_stage/P_terminal should provide analytical Jacobians"
        )

    def test_cdtracking_ocp_objective_jac_matches_fd(self):
        """ContinuousNonlinearOCP objective gradient matches finite differences."""
        model = ScalarNonlinear()
        ocp = ContinuousNonlinearOCP(
            model, N=2,
            Q_z=np.eye(1) * 2.0,
        ...
        np.testing.assert_allclose(grad_ana, grad_fd, atol=1e-4, rtol=1e-4,
                                   err_msg="ContinuousNonlinearOCP objective gradient mismatch")
    ...
    def test_solve_produces_same_result_with_analytical_jac(self):
        ...
        ocp_nograd = ContinuousNonlinearOCP(
            model, N=3,
        ...
        ocp_grad = ContinuousNonlinearOCP(
            model, N=3,
            lagrange=lambda t, x, y, u, p: float(u @ u),
...
class TestCDLinearizedMPCController:
    ...
    def test_relinearizes_each_step(self):
        ...
        ctrl = CDLinearizedMPCController(
            model=model, estimator=ekf, N=5, Q=np.eye(1) * 2.0, R=np.eye(1) * 0.1, dt=1.0,
    ...
    def test_closed_loop_moves_toward_reference(self):
        ...
        ctrl = CDLinearizedMPCController(
            model=model, estimator=ekf, N=10, Q=np.eye(1) * 8.0, R=np.eye(1) * 0.05, dt=1.0,
