"""
Discrete-time linear tracking OCP (QP formulation).

``DiscreteLinearOCP``
    Receding-horizon QP with hard input and soft output box constraints for
    linear discrete-time plant models (:class:`~mbc.models.DiscreteLinearSDE`).
...
from ._base import DiscreteLinearOCPBase
...
class DiscreteLinearOCP(DiscreteLinearOCPBase):
    """
    ...
    solver : str or QPSolverBackend, optional
        ...
        OSQP solver; ``"highs"`` uses the MIT-licensed HiGHS solver.
    ...
    u_min : (nu,) array-like, optional
        Override lower input bounds (overrides ``model.u_bounds[0]``).
    u_max : (nu,) array-like, optional
        Override upper input bounds (overrides ``model.u_bounds[1]``).
    """
    ...
    def u_min(self) -> np.ndarray:
        """Lower input bounds (falls back to model.u_bounds[0] if not set)."""
        return self._u_min if self._u_min is not None else _any_to_np1d(self._model.u_bounds[0])
    ...
    def u_max(self) -> np.ndarray:
        """Upper input bounds (falls back to model.u_bounds[1] if not set)."""
        return self._u_max if self._u_max is not None else _any_to_np1d(self._model.u_bounds[1])
