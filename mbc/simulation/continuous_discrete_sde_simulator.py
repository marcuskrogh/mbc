"""
Numerical integrator for continuous-discrete SDE models
(ControlToolbox §SDE — *Numerical Integration*).

Implements two integration schemes for the Itô SDE

    dx(t) = f(x, u, d, p, t) dt + sigma(x, u, d, p, t) dw(t),
    dw ~ N(0, I dt).

over an interval ``[t_k, t_{k+1}]`` of length ``Ts`` partitioned into
``N`` equidistant sub-steps of size ``Δt = Ts / N`` with conventions
(ControlToolbox §SDE):

    x_{k,n} ≈ x(t_k + n Δt),
    u_{k,n} = u_k                       (zero-order hold)
    d_{k,n} = d_k                       (zero-order hold)
    Δω_{k,n} = z_{k,n} √Δt,  z_{k,n} ~ N(0, I)

**Explicit Euler-Maruyama** (``IntegrationScheme.EXPLICIT_EULER``) — both
drift and diffusion evaluated at the current sub-step:

    x_{k,n+1} = x_{k,n} + f(x_{k,n}, u_k, d_k, p, t_{k,n}) Δt
                       + sigma(x_{k,n}, u_k, d_k, p, t_{k,n}) Δω_{k,n}

**Implicit-Explicit** (``IntegrationScheme.IMPLICIT_EULER``) — drift
evaluated at the *next* sub-step (implicit), diffusion evaluated explicitly:

    x_{k,n+1} = x_{k,n} + f(x_{k,n+1}, u_k, d_k, p, t_{k,n+1}) Δt
                       + sigma(x_{k,n}, u_k, d_k, p, t_{k,n}) Δω_{k,n}

The implicit equation is solved by Newton's method on the residual

    R(x_{k,n+1}) = x_{k,n+1} − x_{k,n}
                   − f(x_{k,n+1}, u_k, d_k, p, t_{k,n+1}) Δt
                   − sigma(x_{k,n}, u_k, d_k, p, t_{k,n}) Δω_{k,n} = 0,

with Jacobian

    ∂R/∂x = I − (∂f/∂x)(x_{k,n+1}, …) Δt.

Use the explicit scheme when the drift dynamics are non-stiff and the
implicit scheme when stiff.
"""

from __future__ import annotations

from dataclasses import dataclass

import numpy as np

from ..models import ContinuousDiscreteSDE
from ..estimation._base import IntegrationScheme
from .._utils import _newton_solve
from ._base import ContinuousDiscreteSimulator, SimulatorParams


# ── Parameter structure ───────────────────────────────────────────────────────


@dataclass
class ContinuousDiscreteSDESimulatorParams(SimulatorParams):
    """
    Algorithm parameters for :class:`ContinuousDiscreteSDESimulator`.

    Parameters
    ----------
    n_steps : int
        Number of integration sub-steps per measurement interval.
        Default: 10.
    scheme : IntegrationScheme
        Integration scheme.  :attr:`~IntegrationScheme.EXPLICIT_EULER`
        (default) is explicit Euler-Maruyama;
        :attr:`~IntegrationScheme.IMPLICIT_EULER` uses an implicit drift
        solve and is suitable for stiff dynamics.
    seed : int or None
        Random seed for reproducibility.  Default: None.
    newton_tol : float
        Newton tolerance for the implicit drift solve.
        Ignored when ``scheme=IntegrationScheme.EXPLICIT_EULER``.
        Default: 1e-12.
    newton_max_iter : int
        Maximum Newton iterations for the implicit drift solve.
        Ignored when ``scheme=IntegrationScheme.EXPLICIT_EULER``.
        Default: 50.
    """
    n_steps: int = 10
    scheme: IntegrationScheme = IntegrationScheme.EXPLICIT_EULER
    seed: int | None = None
    newton_tol: float = 1e-12
    newton_max_iter: int = 50


# ── Simulator ─────────────────────────────────────────────────────────────────


class ContinuousDiscreteSDESimulator(ContinuousDiscreteSimulator):
    """
    Euler-Maruyama / implicit-explicit simulator for continuous-discrete
    SDE models (ControlToolbox §SDE).

    Parameters
    ----------
    model : ContinuousDiscreteSDE
        The nonlinear SDE system to simulate.  Must expose a ``Ts``
        property giving the measurement sampling interval (seconds).
    params : ContinuousDiscreteSDESimulatorParams, optional
        Algorithm parameter struct.  Pass to control ``n_steps``, ``scheme``,
        random seed, and Newton solver settings.
    """

    def __init__(
        self,
        model: ContinuousDiscreteSDE,
        params: ContinuousDiscreteSDESimulatorParams | None = None,
    ) -> None:
        if params is None:
            params = ContinuousDiscreteSDESimulatorParams()
        if not isinstance(params.scheme, IntegrationScheme):
            raise TypeError(
                f"scheme must be an IntegrationScheme member, got {params.scheme!r}."
            )

        self._model = model
        self._Ts = float(model.Ts)
        self._n_steps = int(params.n_steps)
        self._scheme = params.scheme
        self._rng = np.random.default_rng(params.seed)
        self._newton_tol = params.newton_tol
        self._newton_max_iter = params.newton_max_iter

    def step(
        self,
        x: np.ndarray,
        u: np.ndarray,
        d: np.ndarray,
        p: np.ndarray,
        t: float,
    ) -> np.ndarray:
        """
        Simulate one measurement interval from t to t + Ts.

        Applies ``n_steps`` sub-steps with step size ``Δt = Ts / n_steps``.
        The input u and disturbance d are held constant over the interval
        (zero-order hold).

        Parameters
        ----------
        x : (nx,) ndarray  — state at time t.
        u : (nu,) ndarray  — control input over [t, t+Ts].
        d : (nd,) ndarray  — disturbance over [t, t+Ts].
        p : (nparams,) ndarray  — parameter vector.
        t : float          — current time.

        Returns
        -------
        x_next : (nx,) state at t + Ts (one realisation).
        """
        h = self._Ts / self._n_steps
        sqrt_h = np.sqrt(h)
        nx = x.shape[0]

        x_cur = x.copy()
        t_cur = t

        for _ in range(self._n_steps):
            sigma_val = self._model.sigma(x_cur, u, d, p, t_cur)  # (nx, nw)
            nw = sigma_val.shape[1]
            z = self._rng.standard_normal(nw)
            noise = sigma_val @ z * sqrt_h

            if self._scheme is IntegrationScheme.EXPLICIT_EULER:
                f_val = self._model.f(x_cur, u, d, p, t_cur)
                x_cur = x_cur + f_val * h + noise
            else:  # IMPLICIT_EULER
                rhs = x_cur + noise
                t_next = t_cur + h

                def residual(xn: np.ndarray) -> np.ndarray:
                    return xn - rhs - self._model.f(xn, u, d, p, t_next) * h

                def jacobian(xn: np.ndarray) -> np.ndarray:
                    return np.eye(nx) - h * self._model.dfdx(xn, u, d, p, t_next)

                x_cur = _newton_solve(
                    residual, jacobian, x_cur.copy(),
                    tol=self._newton_tol, max_iter=self._newton_max_iter,
                )

            t_cur += h

        return x_cur

    def simulate(
        self,
        x0: np.ndarray,
        U: np.ndarray,
        D: np.ndarray,
        P: np.ndarray,
        t0: float = 0.0,
    ) -> np.ndarray:
        """
        Simulate over a full horizon of T measurement intervals.

        Parameters
        ----------
        x0 : (nx,) ndarray
            Initial state at time t0.
        U : (T, nu) ndarray
            Input trajectory; U[k] is applied over [t_k, t_{k+1}].
        D : (T, nd) ndarray
            Disturbance trajectory; D[k] applies over [t_k, t_{k+1}].
        P : (T, nparams) ndarray
            Parameter trajectory; P[k] applies over [t_k, t_{k+1}].
        t0 : float, optional
            Start time.  Default: 0.

        Returns
        -------
        X : (T+1, nx) ndarray
            State trajectory where X[0] = x0 and X[k+1] = step(X[k], ...).
        """
        T = U.shape[0]
        nx = x0.shape[0]
        X = np.empty((T + 1, nx))
        X[0] = x0.copy()
        t_cur = t0
        for k in range(T):
            X[k + 1] = self.step(X[k], U[k], D[k], P[k], t_cur)
            t_cur += self._Ts
        return X
