...
`ContinuousDiscreteParticleFilter`) and by `ContinuousDiscreteSDESimulator` and `EconomicOptimalControlProblem`.
...
Accepted by `ContinuousDiscreteSDAESimulator` and `ContinuousDiscreteDAEEKF`.
...
#### `ContinuousDiscreteSDESimulator` — `mbc.simulation`
...
`t_{k+1} = t_k + Ts` using `n_steps` equidistant sub-steps of size
`Δt = Ts / n_steps`.  Inputs `u` and disturbances `d` are held constant
(zero-order hold) over each measurement interval.  The discrete Wiener
increment is `Δω_n = z_n √Δt` with `z_n ~ N(0, I)`.

**Explicit-Explicit Euler-Maruyama (EXPLICIT_EULER)** — both drift and diffusion
...
**Implicit-Explicit (IMPLICIT_EULER) scheme** — drift evaluated at the *next*
...
The IMPLICIT_EULER scheme is appropriate when the drift dynamics are stiff (large
eigenvalues in `∂f/∂x`).

**Constructor**: `ContinuousDiscreteSDESimulator(model, params=None)`

The sampling interval `Ts` is read from `model.Ts`.  Algorithm parameters
are passed via a `ContinuousDiscreteSDESimulatorParams` dataclass:

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `n_steps` | `int` | `10` | Integration sub-steps per interval |
| `scheme` | `IntegrationScheme` | `EXPLICIT_EULER` | Integration scheme |
| `seed` | `int` or `None` | `None` | Random seed for reproducibility |
| `newton_tol` | `float` | `1e-12` | IMPLICIT_EULER drift Newton tolerance |
| `newton_max_iter` | `int` | `50` | IMPLICIT_EULER maximum Newton iterations per sub-step |

**Methods**:

```python
from mbc.simulation import ContinuousDiscreteSDESimulator, ContinuousDiscreteSDESimulatorParams
from mbc.estimation import IntegrationScheme

params = ContinuousDiscreteSDESimulatorParams(n_steps=20, seed=42)
sim = ContinuousDiscreteSDESimulator(model, params=params)

# Simulate one measurement interval [t, t+Ts]
...
#### `ContinuousDiscreteSDAESimulator` — `mbc.simulation`
...
**Constructor**: `ContinuousDiscreteSDAESimulator(model, params=None)`

The sampling interval `Ts` is read from `model.Ts`.  Algorithm parameters
are passed via a `ContinuousDiscreteSDAESimulatorParams` dataclass:
...
```python
from mbc.simulation import ContinuousDiscreteSDAESimulator, ContinuousDiscreteSDAESimulatorParams

params = ContinuousDiscreteSDAESimulatorParams(n_steps=20, newton_tol=1e-10)
sim = ContinuousDiscreteSDAESimulator(model, params=params)
...
To simulate stiff SDE dynamics use [`ContinuousDiscreteSDESimulator`](#continuousdiscretesdesimulator----mbcsimulation)
with `scheme=IntegrationScheme.IMPLICIT_EULER`; that choice is independent of the filter.
...
| `simulator` | `ContinuousDiscreteSDESimulator` or `ContinuousDiscreteSDAESimulator` | — | Plant dynamics integrator |
